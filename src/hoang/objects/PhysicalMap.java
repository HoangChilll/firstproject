...
public class PhysicalMap extends GameObject{
    ...
    public void draw(Graphics2D g2){
        ...
        for(int i =0;i< phys_map.length;i++){
            for(int j =0;j<phys_map[0].length;j++){
                if(phys_map[i][j]!=0) g2.fillRect((int)getPosX()+j*titleSize-(int)camera.getPosX(),(int)getPosY()+i*titleSize-(int)camera.getPosY(),titleSize,titleSize);
            }
        }
    }
    public Rectangle haveCollsionWithland(Rectangle rect){
        ...
        if(posX1<0) posX1=0;
        if(posX2>=phys_map[0].length) posX2=phys_map.length-1;
    ...
    public Rectangle haveCollsionWithTop(Rectangle rect){
        int posX1=rect.x/titleSize;
        posX1-=2;
        int posX2=(rect.x+ rect.width)/titleSize;
        posX2+=2;
    ...
    public Rectangle haveCollsionWithRightWall(Rectangle rect){
        int posY1= rect.y/titleSize;
        posY1-=2;
    ...
    public Rectangle haveCollsionWithLeftWall(Rectangle rect){
        int posY1= rect.y/titleSize;
        posY1-=2;
        int posY2=(rect.y+ rect.height)/titleSize;
        posY2+=2;
